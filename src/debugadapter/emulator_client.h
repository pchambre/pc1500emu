...
//
// Client for a running pc1500emu's scriptable command interface (see
// main.cpp's "Scriptable command interface" section and README.md) -- a
// Win32 named pipe on Windows, a POSIX FIFO everywhere else, matching
// main.cpp's own dual implementation exactly. Reimplements what
// tools/send-command.ps1 does, in C++, so the debug adapter's request/
// response loop doesn't pay PowerShell's process-launch latency on every
// single-step.
#pragma once
...
class EmulatorClient {
 public:
  ...
  // over the emulator's command pipe/FIFO to a running instance, then
  // waits for its response, written to the fixed pc1500emu.response file
  // (%TEMP% on Windows, /tmp on POSIX), to be rewritten -- detected via
  // the file's last-write time rather than its content, since two
